...
//《——————————————————socket模块结束————————————————————-》
    ...
    //视角控制--》状态及数据传输
    $('#camera_control li').click(function(){
        ...
        //设置父容器的属性data-type为点击视角对应的data-type
    ...
    $("#middleCell").on("click",".prolist_content li",function(){
        //状态控制
        sel_status($(this));
        ...
        var $this = $(this);
        var _type = $this.attr('data-type');
        var simulation_id = $this.parents('#special_simulation_option').attr('data-simulation_id');
        ...
        if(socket){
            if(_type == 1){
                //数据引擎-GPS卫星
                socket.emit('control_screenGps',{'on_off':_type});
            }
            if(_type == 2){
                //大屏-CPDLC
                socket.emit('control_screenCpdlc',{'on_off':_type});
            }
            if(_type == 3){
                //大屏-起飞机场
                socket.emit('control_screenFly',{'on_off':_type});
            }
            if(_type == 4){
                //大屏-标题信息
                socket.emit('control_screenTitle',{'on_off':_type});
            }
            if(_type == 5){
                //大屏-三维视景
                socket.emit('control_screenThree',{'on_off':_type});
            }
            if(_type == 6){
                //大屏-飞行剖面
                socket.emit('control_screenFplan',{'on_off':_type});
            }
            if(_type == 6){
                //大屏-降落机场
                socket.emit('control_screenDown',{'on_off':_type});
            }
        }
    });

    //天气部分--》状态及数据传输
    $('#weather_control li').click(function(){
    ...
    $("#affectCell").on("click",".prolist_content2 li",function(){
        //状态控制
        sel_status($(this));
        ...
        var $this = $(this);
        var _type = $this.attr('data-type');
        var effects_id = $this.parents('#special_effects_option').attr('data-ffects_id');
        // var plane_id = $this.parents('#special_effects_option').attr('data-plane_id');
        $this.parents('#special_effects_option').attr('data-type',_type);
    ...
    $("#window_control li").click(function(){
        ...
        var index =  $("#window_control li").index(this);  // 获取当前点击的<li>元素 在 全部li元素中的索引。
        $("div.tab_box > ol").eq(index).show().siblings().hide();

        if(index == 1){
            sel_win($("#source_control li"));
        }
        if(index == 2){
            sel_win($("#source_control2 li"));
        }
        if(index == 3){
            sel_win($("#source_control3 li"));
        }
        if(index == 4){
            sel_win($("#source_control4 li"));
        }
        if(index == 5){
            sel_win($("#source_control5 li"));
        }

    })

    function  sel_win(model){
       ...
       $(model).click(function(){
           ...
           var ol =  $this.parents('#source_control'); //获取当前点击元素父元素
           var olAll = $("div.tab_box > ol");  //所有ol
           var index =  olAll.index(ol);   //获取当前点击元素父元素ol在所有ol中的索引

           var win = $(model).eq(index); //获取对应索引的选项window元素
           win.attr('data-value',_type);
    ...
    function sel_status(model){
        if(model.hasClass('selected ')){
            model.removeClass('selected ');
            return;
        }
        model.toggleClass('selected ');
    }
    function sel_status2(model){
...
/*$(function(){
    ...
    linum2 = $('.mainlist2 li').length;

    w2 = linum2/2 * 303;//ul宽度
    ...
    $('.drama-slide2 li.next2 a').click(function(){
        ...
        if($('.mainlist2 li').length>3){
            ...
            if(ml2<=0 && ml2>w2*-1){//默认图片显示时
                ...
                $('.mainlist2').animate({left: ml2 - 909 + 'px'},'slow');//默认图片滚动
                if(ml2==(w2-909)*-1){//默认图片最后一屏时
                    $('.swaplist2').animate({left: '0px'},'slow');//交换图片滚动
                }
            }else{//交换图片显示时
                $('.mainlist2').css({left: '948px'})//默认图片放在显示区域右
                $('.swaplist2').animate({left: sl2 - 909 + 'px'},'slow');//交换图片滚动
                if(sl2==(w2-909)*-1){//交换图片最后一屏时
    ...
    $('.drama-slide2 li.prev2 a').click(function(){
        ...
        if($('.mainlist2 li').length>3){
            ...
            if(ml2<=0 && ml2>w2*-1){
                ...
                $('.mainlist2').animate({left: ml2 + 909 + 'px'},'slow');
                if(ml2==0){
                    $('.swaplist2').animate({left: (w2 - 909) * -1 + 'px'},'slow');
                }
            }else{
                $('.mainlist2').css({left: (w2 - 909) * -1 + 'px'});
                $('.swaplist2').animate({left: sl2 + 909 + 'px'},'slow');
                if(sl==0){
...
$(function(){
    //页数切换扩展
     ...
     var none_unit_width = $(".prolist_content").width();  //最外容器固定宽度，可以不用获取
     var none_unit_height = $(".prolist_content").height()-20;
     var $parent = $(".mainlist"); //获取父元素

    //$(".prolist_content ul>li:nth-child(8n)").css("background","transparent");
    ...
    $(".drama-slide li.next a").click(function(){
        if( !$parent.is(":animated") ){
            ...
            if( page == page_count ){  //已经到最后一个版面了,如果再向后，必须跳转到第一个版面。
                $parent.animate({ top : 0}, 200); //通过改变top值，跳转到第一个版面
               // $parent.stop().animate();
               // $(this).unbind();
                page = 1;
            }else{
                $parent.animate({ top : '-='+none_unit_height}, 200);  //通过改变top值，达到每次换一个版面
                page++;
            }
        }
    });
   ...
   $(".drama-slide li.prev a").click(function(){
        if( !$parent.is(":animated") ){
              ...
              if( page == 1 ){  //已经到第一个版面了,如果再向前，必须跳转到最后一个版面。
                   $parent.animate({ top : '-='+none_unit_height*(page_count-1)}, 200); //通过改变top值，跳转到最后一个版面
                   page = page_count;
               }else{
                   $parent.animate({ top : '+='+none_unit_height }, 200);  //通过改变left值，达到每次换一个版面
                   page--;
               }
        }
    });
...
$(function(){
    ...
    var page_count = Math.ceil(len / i) ;     //页数
    var none_unit_width = $(".prolist_content2").width();  //最外容器固定宽度，可以不用获取
    var none_unit_height = $(".prolist_content2").height()-17;
    var $parent = $(".mainlist2"); //获取父元素
