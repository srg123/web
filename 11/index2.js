...
*/
  ...
  /*  根据飞机滑动距离左面的宽度来决定时间点出现的位置来决定时间轴上点的样式
    ...
    console.log('.control-button-layout li::after');
    //布局模块
    $('.control-button-layout li').click(function(){
        ...
        var $this = $(this);
        var _type = $this.attr('data-type');
        var layout_id = $this.parents('#layout_control').attr('data-layout_id');
        var plane_id = $this.parents('#layout_control').attr('data-plane_id');

        // alert($this.siblings('.selected').length)
        $this.parents('#layout_control').attr('data-type',_type);
        ...
        if(socket){
            socket.emit('control_layout_change',{
                'type':_type,
                'layout_id':layout_id,
                'plane_id':plane_id
            });
